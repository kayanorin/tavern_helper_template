...
const GMA_NOTICES: Record<string, string> = {
  ...
  风味漫步: '📖 GMA系统通知：已认证「风味漫步」，解锁基础资料库权限',
  百味巡礼者: '🏅 GMA系统通知：已认证「百味巡礼者」，参与活动餐厅消费享9折优惠',
  巡礼半程: '🔬 GMA系统通知：已认证「巡礼半程」，解锁GMA内部资料库权限',
  味觉探索家: '⭐ GMA系统通知：已认证「味觉探索家」，后续所有在百城巡礼中的路费、住宿费减半',
...
export function init(): () => void {
  ...
  // ─── 里程碑称号弹窗 ───
  const sub1 = eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (new_variables: any, old_variables: any) => {
    const newTitles: string[] = _.get(new_variables, 'stat_data.巡礼.称号') ?? [];
    const oldTitles: string[] = _.get(old_variables, 'stat_data.巡礼.称号') ?? [];
  ...
  // ─── NPC 生命周期管理 ───
  const sub2 = eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (new_variables: any, old_variables: any) => {
    const currentFloor = getLastMessageId();
    const newInScene: string[] = _.get(new_variables, 'stat_data.巡礼.在场角色') ?? [];
    const oldInScene: string[] = _.get(old_variables, 'stat_data.巡礼.在场角色') ?? [];
    const retireList: string[] = _.get(new_variables, 'stat_data.巡礼.退赛NPC') ?? [];
    const npcBoard: Record<string, any> = _.get(new_variables, 'stat_data.$NPC榜单') ?? {};
    ...
    for (const name of entered) {
      if (!npcBoard[name]) {
        ...
        _.set(new_variables, `stat_data.$NPC榜单.${name}.离场楼层`, 0);
      }
    }
    ...
    for (const name of left) {
      if (!retireList.includes(name) && npcBoard[name]) {
        _.set(new_variables, `stat_data.$NPC榜单.${name}.离场楼层`, currentFloor);
      }
    }
  ...
  // ─── NPC 随机评分事件 ───
  const sub3 = eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (new_variables: any, old_variables: any) => {
    const currentFloor = getLastMessageId();
